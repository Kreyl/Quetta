        ...
        for(uint32_t n=0; n < 0x10; n++) {
            ...
            if(Addr <= 0x01 or Addr > 0x77) Printf("   ");
                ...
                for(uint32_t t=0; t<9000; t++) {
                    if(pi2c->ISR & I2C_ISR_STOPF) break;
                }
...
uint8_t i2c_t::CheckAddress(uint32_t Addr) {
    ...
    IReset(); // Reset I2C
    uint8_t Rslt;
    uint32_t RetryCnt = 360; // Experimental
    I2C_TypeDef *pi2c = PParams->pi2c;  // To make things shorter
    if(IBusyWait() != retvOk) {
        Rslt = retvBusy;
        Printf("i2cC Busy\r");
...
uint8_t i2c_t::Write(uint8_t Addr, uint8_t *WPtr, uint32_t WLength) {
#if I2C_USE_SEMAPHORE
    if(chBSemWait(&BSemaphore) != MSG_OK) return FAILURE;
#endif
    uint8_t Rslt;
...
uint8_t i2c_t::WriteRead(uint8_t Addr, uint8_t *WPtr, uint32_t WLength, uint8_t *RPtr, uint32_t RLength) {
#if I2C_USE_SEMAPHORE
    if(chBSemWait(&BSemaphore) != MSG_OK) return FAILURE;
#endif
    uint8_t Rslt;
...
uint8_t i2c_t::CheckBusAndResume() {
    PParams->pi2c->CR1 &= ~I2C_CR1_PE;
    PinSetupInput(PParams->PGpio, PParams->SclPin, pudPullUp);
    PinSetupInput(PParams->PGpio, PParams->SdaPin, pudPullUp);
    chThdSleepMilliseconds(2);
    uint8_t Rslt = (PinIsHi(PParams->PGpio, PParams->SclPin) and PinIsHi(PParams->PGpio, PParams->SdaPin))? retvOk : retvFail;
    Resume();
    return Rslt;
}
