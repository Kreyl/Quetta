/*
 * kl_crypto.cpp
 *
 *  Created on: 28 июн. 2023 г.
 *      Author: laurelindo
 */

#include "kl_crypto.h"
#include <cstring> // memcpy

namespace Sha256 {

void Init(Buff_t *pBuf) {
    pBuf->h[0] = 0x6a09e667;
    pBuf->h[1] = 0xbb67ae85;
    pBuf->h[2] = 0x3c6ef372;
    pBuf->h[3] = 0xa54ff53a;
    pBuf->h[4] = 0x510e527f;
    pBuf->h[5] = 0x9b05688c;
    pBuf->h[6] = 0x1f83d9ab;
    pBuf->h[7] = 0x5be0cd19;
    pBuf->DataSz = 0;
    pBuf->ChunkSz = 0;
}

static const uint32_t k[64] = {
    0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
    0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
    0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
    0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
    0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
    0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
    0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
    0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2
};

#define rotate_r(val, bits) (val >> bits | val << (32 - bits))

static void CalcChunk(Buff_t *pBuf, const uint8_t* chunk) {
    uint32_t w[64];
    uint32_t tv[8];
    uint32_t i;

    for (i=0; i<16; ++i){
        w[i] = (uint32_t) chunk[0] << 24 | (uint32_t) chunk[1] << 16 | (uint32_t) chunk[2] << 8 | (uint32_t) chunk[3];
        chunk += 4;
    }

    for (i=16; i<64; ++i){
        uint32_t s0 = rotate_r(w[i-15], 7) ^ rotate_r(w[i-15], 18) ^ (w[i-15] >> 3);
        uint32_t s1 = rotate_r(w[i-2], 17) ^ rotate_r(w[i-2], 19) ^ (w[i-2] >> 10);
        w[i] = w[i-16] + s0 + w[i-7] + s1;
    }

    for (i = 0; i < 8; ++i)
        tv[i] = pBuf->h[i];

    for (i=0; i<64; ++i){
        uint32_t S1 = rotate_r(tv[4], 6) ^ rotate_r(tv[4], 11) ^ rotate_r(tv[4], 25);
        uint32_t ch = (tv[4] & tv[5]) ^ (~tv[4] & tv[6]);
        uint32_t temp1 = tv[7] + S1 + ch + k[i] + w[i];
        uint32_t S0 = rotate_r(tv[0], 2) ^ rotate_r(tv[0], 13) ^ rotate_r(tv[0], 22);
        uint32_t maj = (tv[0] & tv[1]) ^ (tv[0] & tv[2]) ^ (tv[1] & tv[2]);
        uint32_t temp2 = S0 + maj;

        tv[7] = tv[6];
        tv[6] = tv[5];
        tv[5] = tv[4];
        tv[4] = tv[3] + temp1;
        tv[3] = tv[2];
        tv[2] = tv[1];
        tv[1] = tv[0];
        tv[0] = temp1 + temp2;
    }

    for (i = 0; i < 8; ++i) pBuf->h[i] += tv[i];
}


void Update(Buff_t *pBuf, const void* pData, uint32_t Sz) {
    const uint8_t* ptr = (const uint8_t*)pData;
    pBuf->DataSz += Sz;
    // If there is data left in buff, concatenate it to process as new chunk
    if (Sz + pBuf->ChunkSz >= 64) {
        uint8_t tmp_chunk[64];
        memcpy(tmp_chunk, pBuf->LastChunk, pBuf->ChunkSz);
        memcpy(tmp_chunk + pBuf->ChunkSz, ptr, 64 - pBuf->ChunkSz);
        ptr += (64 - pBuf->ChunkSz);
        Sz -= (64 - pBuf->ChunkSz);
        pBuf->ChunkSz = 0;
        CalcChunk(pBuf, tmp_chunk);
    }
    // Run over data chunks
    while (Sz  >= 64) {
        CalcChunk(pBuf, ptr);
        ptr += 64;
        Sz -= 64;
    }
    // Save remaining data in buff, will be reused on next call or finalize
    memcpy(pBuf->LastChunk + pBuf->ChunkSz, ptr, Sz);
    pBuf->ChunkSz += Sz;
}

void Finalize(Buff_t *pBuf) {
    pBuf->LastChunk[pBuf->ChunkSz] = 0x80;
    pBuf->ChunkSz++;
    memset(pBuf->LastChunk + pBuf->ChunkSz, 0, 64 - pBuf->ChunkSz);

    /* If there isn't enough space to fit int64, pad chunk with zeroes and prepare next chunk */
    if (pBuf->ChunkSz > 56) {
        CalcChunk(pBuf, pBuf->LastChunk);
        memset(pBuf->LastChunk, 0, 64);
    }

    /* Add total size as big-endian int64 x8 */
    uint64_t size = pBuf->DataSz * 8;
    int i;
    for (i = 8; i > 0; --i) {
        pBuf->LastChunk[55+i] = size & 255;
        size >>= 8;
    }

    CalcChunk(pBuf, pBuf->LastChunk);
}


void Read2BinArr(Buff_t *pBuf, uint8_t* Hash) {
    uint32_t i;
    for (i = 0; i < 8; i++) {
        Hash[i*4]     = (pBuf->h[i] >> 24) & 0xFF;
        Hash[i*4 + 1] = (pBuf->h[i] >> 16) & 0xFF;
        Hash[i*4 + 2] = (pBuf->h[i] >> 8)  & 0xFF;
        Hash[i*4 + 3] =  pBuf->h[i]        & 0xFF;
    }
}


void Hash2BinArr(const void *pData, uint32_t Sz, uint8_t* Hash) {
    Buff_t buff;
    Init(&buff);
    Update(&buff, pData, Sz);
    Finalize(&buff);
    Read2BinArr(&buff, Hash);
}

} // namespace Sha256

