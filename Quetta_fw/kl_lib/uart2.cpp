...
void BaseUart_t::IRQDmaTxHandler() {
    ...
    IFullSlotsCount = IFullSlotsCount - ITransSize;
    PRead += ITransSize;
...
void BaseUart_t::ISendViaDMA() {
    ...
    if(ITransSize != 0) {
        ...
        dmaStreamSetMode(PDmaTx, DmaModeTx);
...
uint8_t BaseUart_t::IPutByte(uint8_t b) {
    ...
    *PWrite++ = b;
    ...
    IFullSlotsCount = IFullSlotsCount + 1;
    return retvOk;
}
...
void BaseUart_t::Init() {
    ...
    PinAF = AF7;
#if defined UART4
    if(Uart == UART4) PinAF = AF8;
#endif
#if defined UART5
    if(Uart == UART5) PinAF = AF8;
#endif
#if defined USART6
    if(Uart == USART6) PinAF = AF8;
#endif

#elif defined STM32F0XX
    if(PGpioTx == GPIOA) PinAF = AF1;
    else if(PGpioTx == GPIOB) PinAF = AF0;
#elif defined STM32F7XX
    if(Uart == USART1) { // AF4, AF7
        if(PGpioTx == GPIOB and PinTx == 14) PinAF = AF4;
        else PinAF = AF7;
    }
    else if(Uart == USART2 or Uart == USART3) PinAF = AF7;
    else if(Uart == UART4) { // AF6, AF8
        if(PGpioTx == GPIOA and PinTx == 12) PinAF = AF6;
        else PinAF = AF8;
    }
    else if(Uart == UART5) { // AF1, AF7, AF8
        if(PGpioTx == GPIOB and PinTx == 6) PinAF = AF1;
        else if(PGpioTx == GPIOB and PinTx == 9) PinAF = AF7;
        else PinAF = AF8;
    }
    else if(Uart == USART6 or Uart == UART8) PinAF = AF8;
    else if(Uart == UART7) { // AF8, AF12
        if(PGpioTx == GPIOA or PGpioTx == GPIOB) PinAF = AF12;
        else PinAF = AF8;
    }
#else
#error "UART AF not defined"
#endif
    PinSetupAlterFunc((GPIO_TypeDef*)PGpioTx, PinTx, omPushPull, pudNone, PinAF);
#endif

#if 1 // Setup independent clock if possible and required
#if defined STM32F072xB
    if(UseIndependedClock) {
    ...
    if(Uart == USART2) Offset = 2;
    else if(Uart == USART3) Offset = 4;
    else if(Uart == UART4) Offset = 6;
    else if(Uart == UART5) Offset = 8;
    RCC->CCIPR &= ~(0b11UL << Offset); // Clear current bits
    RCC->CCIPR |= ((uint32_t)ClkSrc) << Offset;
    // Enable HSI if needed
    if(ClkSrc == uartclkHSI) Clk.EnableHSI();
#elif defined STM32F7XX
    uint32_t Offset = 0; // Usart1
    if(Uart == USART2) Offset = 2;
    else if(Uart == USART3) Offset = 4;
    ...
    else if(Uart == USART6) Offset = 10;
    else if(Uart == UART7) Offset = 12;
    else if(Uart == UART8) Offset = 14;
    RCC->DCKCFGR2 &= ~(0b11UL << Offset); // Clear current bits
    RCC->DCKCFGR2 |= ((uint32_t)ClkSrc) << Offset;
    // Enable HSI if needed
    if(ClkSrc == uartclkHSI) Clk.EnableHSI();
#endif
...
#if 1 // ==== Clock ====
    if     (Uart == USART1) { rccEnableUSART1(FALSE); IUart1 = this; }
    else if(Uart == USART2) { rccEnableUSART2(FALSE); IUart2 = this; }
#if defined USART3
    else if(Uart == USART3) { rccEnableUSART3(FALSE); IUart3 = this; }
#endif
#if defined UART4
    else if(Uart == UART4) { rccEnableUART4(FALSE); IUart4 = this; }
#endif
#if defined UART5
    else if(Uart == UART5) { rccEnableUART5(FALSE); IUart5 = this; }
#endif
#if defined USART6
    else if(Uart == USART6) { rccEnableUSART6(FALSE); }
#endif
#if defined UART7
    else if(Uart == UART7) { rccEnableUART7(FALSE); }
#endif
#if defined UART8
    else if(Uart == UART8) { rccEnableUART8(FALSE); }
#endif
#endif // Clock

    OnClkChange();  // Setup baudrate

    Uart->CR2 = 0;  // Nothing that interesting there
    // ==== DMA ====
    // Remap DMA request if needed
#if defined STM32F0XX
    if(PDmaTx == STM32_DMA1_STREAM4) SYSCFG->CFGR1 |= SYSCFG_CFGR1_USART1TX_DMA_RMP;
#endif
    PDmaTx = dmaStreamAlloc(DmaTxID, IRQ_PRIO_MEDIUM, DmaUartTxIrq, this);
    dmaStreamSetPeripheral(PDmaTx, &Uart->TDR);
    dmaStreamSetMode      (PDmaTx, DmaModeTx);
    IDmaIsIdle = true;
...
#endif
#elif defined STM32F0XX
    if(PGpioRx == GPIOA) PinAF = AF1;
    else if(PGpioRx == GPIOB) PinAF = AF0;
#elif defined STM32F7XX
    if(Uart == USART1) { // AF4, AF7
        if(PGpioTx == GPIOB and PinTx == 15) PinAF = AF4;
        else PinAF = AF7;
    }
    else if(Uart == USART2 or Uart == USART3) PinAF = AF7;
    else if(Uart == UART4) { // AF6, AF8
        if(PGpioTx == GPIOA and PinTx == 11) PinAF = AF6;
        else PinAF = AF8;
    }
    else if(Uart == UART5) { // AF1, AF7, AF8
        if(PGpioTx == GPIOB and PinTx == 5) PinAF = AF1;
        else if(PGpioTx == GPIOB and PinTx == 8) PinAF = AF7;
        else PinAF = AF8;
    }
    else if(Uart == USART6 or Uart == UART8) PinAF = AF8;
    else if(Uart == UART7) { // AF8, AF12
...
#error "UART AF not defined"
#endif
    PinSetupAlterFunc((GPIO_TypeDef*)PGpioRx, PinRx, omOpenDrain, pudPullUp, PinAF);
    // Remap DMA request if needed
#if defined STM32F0XX
    if(PDmaRx == STM32_DMA1_STREAM5) SYSCFG->CFGR1 |= SYSCFG_CFGR1_USART1RX_DMA_RMP;
#endif
    // DMA
    PDmaRx = dmaStreamAlloc(DmaRxID, IRQ_PRIO_MEDIUM, nullptr, NULL);
    dmaStreamSetPeripheral(PDmaRx, &Uart->RDR);
    dmaStreamSetMemory0   (PDmaRx, IRxBuf);
    dmaStreamSetTransactionSize(PDmaRx, UART_RXBUF_SZ);
    dmaStreamSetMode      (PDmaRx, DmaModeRx);
    dmaStreamEnable       (PDmaRx);

    // Enable IRQ on <LF>
    Uart->CR2 |= ((uint32_t)('\n')) << 24; // What to recognize
    Uart->CR1 |= USART_CR1_CMIE; // Enable IRQ on match
    if(Uart == USART1) nvicEnableVector(USART1_IRQn, IRQ_PRIO_LOW);
    else if(Uart == USART2) nvicEnableVector(USART2_IRQn, IRQ_PRIO_LOW);
    else if(Uart == USART3) nvicEnableVector(USART3_IRQn, IRQ_PRIO_LOW);
    else if(Uart == UART4) nvicEnableVector(UART4_IRQn, IRQ_PRIO_LOW);
    else if(Uart == UART5) nvicEnableVector(UART5_IRQn, IRQ_PRIO_LOW);
...
void BaseUart_t::SetReplyEndChar(char c) {
    ...
    Uart->CR2 |= ((uint32_t)c) << 24; // What to recognize
    Uart->CR1 |= USART_CR1_UE; // Enable UART
}

void BaseUart_t::Shutdown() {
    Uart->CR1 &= ~USART_CR1_UE; // UART Disable
    if     (Uart == USART1) { rccDisableUSART1(); }
    else if(Uart == USART2) { rccDisableUSART2(); }
#if defined USART3
    else if(Uart == USART3) { rccDisableUSART3(); }
#endif
#if defined UART4
    else if(Uart == UART4) { rccDisableUART4(); }
#endif
#if defined UART5
    else if(Uart == UART5) { rccDisableUART5(); }
#endif
#if defined USART6
    else if(Uart == USART6) { rccDisableUSART6(); }
#endif
#if defined UART7
    else if(Uart == UART7) { rccDisableUART7(); }
#endif
#if defined UART8
    else if(Uart == UART8) { rccDisableUART8(); }
#endif
}

void BaseUart_t::OnClkChange() {
#if defined STM32L1XX || defined STM32F1XX
    if(Uart == USART1) Uart->BRR = Clk.APB2FreqHz / Baudrate;
    else                       Uart->BRR = Clk.APB1FreqHz / Baudrate;
#elif defined STM32F072xB
    if(Uart == USART1 or Uart == USART2) Uart->BRR = HSI_FREQ_HZ / Baudrate;
    else Uart->BRR = Clk.APBFreqHz / Baudrate;
#elif defined STM32F0XX
    Uart->BRR = Clk.APBFreqHz / IBaudrate;
#elif defined STM32F2XX || defined STM32F4XX
    if(Uart == USART1 or Uart == USART6) Uart->BRR = Clk.APB2FreqHz / Baudrate;
    else Uart->BRR = Clk.APB1FreqHz / Baudrate;
#elif defined STM32L4XX
    switch(ClkSrc) {
        case uartclkPCLK:
            ...
            break;
        case uartclkSYSCLK:
            Uart->BRR = Clk.GetSysClkHz() / Baudrate;
            break;
        case uartclkHSI:
            Uart->BRR = HSI_FREQ_HZ / Baudrate;
            break;
        case uartclkLSE:
            Uart->BRR = LSE_FREQ_HZ / Baudrate;
            break;
    } // switch
#elif defined STM32F7XX
    switch(ClkSrc) {
        case uartclkPCLK:
            if(Uart == USART1 or Uart == USART6)
                Uart->BRR = Clk.APB2FreqHz / Baudrate;
            else Uart->BRR = Clk.APB1FreqHz / Baudrate;
            break;
        case uartclkSYSCLK:
            Uart->BRR = Clk.GetSysClkHz() / Baudrate;
...
uint8_t CmdUart_t::TransmitBinaryFromBuf(uint8_t *ptr, uint32_t Len, uint32_t Timeout_ms) {
    ...
    dmaStreamSetMode(PDmaTx, DmaModeTx & (~STM32_DMA_CR_TCIE));
    dmaStreamEnable(PDmaTx);
...
#if 0 // ========================== JSON UART ==================================
void JsonUart_t::OnUartIrqI(uint32_t flags) {
    if(flags & USART_ISR_CMF) {
        EvtQMain.SendNowOrExitI(EvtMsg_t(evtIdShellCmdRcvd, (void*)this));
    }
}

uint8_t JsonUart_t::ReceiveBinaryToBuf(uint8_t *ptr, uint32_t Len, uint32_t Timeout_ms) {
    uint8_t Rslt = retvOk;
    // Wait for previous TX to complete
    while(!IDmaIsIdle);
    while(!(Uart->ISR & USART_ISR_TXE)); // Wait
