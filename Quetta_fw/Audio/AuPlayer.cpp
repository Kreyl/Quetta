...
enum AudioEvt_t {aevtPlay, aevtNewBufReqd};
...
void AuPlayer_t::IPrepareToPlayNext(const char* AFName, PlayMode_t AMode) {
    if(INextSnd->TryOpenFile(AFName) == retvOk) {
        if(INextSnd->Start(AMode, Volume) == retvOk) {
...
void AuPlayer_t::SetVolume(uint32_t AVolume) {
    if(AVolume > VOLUME_MAX) AVolume = VOLUME_MAX;
    AudioTrack::Fade fade_mode = AVolume > Volume? AudioTrack::Fade::LinearIn : AudioTrack::Fade::LinearOut;
    Volume = AVolume;
    ICurSnd->Track.fade((AVolume * AudioTrack::UNIT_LEVEL) / 100, fade_mode, 360);
}

void AuPlayer_t::IncreaseVolume() {
    uint32_t FVolume = Volume;
    if(FVolume < VOLUME_CHANGE_STEP) FVolume = VOLUME_CHANGE_STEP;
    else if(FVolume < VOLUME_MAX) FVolume += VOLUME_CHANGE_STEP;
    SetVolume(FVolume);
    Printf("V: %u\r", Volume);
}

void AuPlayer_t::DecreaseVolume() {
    uint32_t FVolume = Volume;
    if(FVolume == VOLUME_CHANGE_STEP) FVolume = VOLUME_MIN;
    else if(FVolume > VOLUME_CHANGE_STEP) FVolume -= VOLUME_CHANGE_STEP;
    SetVolume(FVolume);
    Printf("V: %u\r", Volume);
}
