...
#define VOLUME_MAX          200UL // max gain = 2
#define VOLUME_MIN          5UL
#define VOLUME_CHANGE_STEP  20UL
...
public:
    ...
    uint8_t Start(PlayMode_t Mode, uint32_t AVolume) {
        return Track.start(&IFile
                , (Mode == spmSingle)? AudioTrack::Mode::Single : AudioTrack::Mode::Continuous
                , true // preload = true
                , (AVolume * AudioTrack::UNIT_LEVEL) / 100 // Start volume
                , AudioTrack::Fade::None // Start immediately
                , 0 // Fade Length
            )? retvOk : retvFail;
    }
...
public:
    ...
    void SetVolume(uint32_t AVolume);
    void IncreaseVolume();
    void DecreaseVolume();
