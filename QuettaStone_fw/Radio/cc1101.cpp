...
cc1101_t CC;
static IrqPin_t IGdo0;
static Thread* PWaitingThread = nullptr;
...
void cc1101_t::TransmitSync(void *Ptr) {
    ...
    EnterTX();
    chSchGoSleepS(THD_STATE_SUSPENDED);    // Wait IRQ
...
uint8_t cc1101_t::ReceiveSync(uint32_t Timeout_ms, void *Ptr, int8_t *PRssi) {
    ...
    PWaitingThread = chThdSelf();
    EnterRX();
    msg_t Rslt = chSchGoSleepTimeoutS(THD_STATE_SUSPENDED, MS2ST(Timeout_ms));    // Wait IRQ
